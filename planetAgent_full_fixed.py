...
# ===== FIX: Call set_page_config() once at the top level of the script =====
...
class LLMExtractor:
    ...
    def extract_and_reply(self, user_message: str, recent_history: list, assistant_state: dict):
        ...
        # ===== FIX: Updated system prompt for more proactive location questions =====
        system_prompt = (
        ...
        if any(phrase in user_low for phrase in need_assume_phrases) and not state.get("geometry"):
            ...
            if place:
                ...
                if geo and geo.get("lat") is not None:
                    ...
                    # ===== FIX: Set half_km to create a ~30km^2 area =====
...
def main():
    ...
    if user_text := st.chat_input("Ask about satellite imagery (dates/cloud/geometry/place)..."):
        ...
        if result.get("status") == "ready":
            ...
            # ===== FIX: Display collected filters in an expander =====
            with st.expander("✅ Filters Collected - Click to see details"):
                st.write(f"**Start Date:** `{filters.get('start_date', 'Not set')}`")
                st.write(f"**End Date:** `{filters.get('end_date', 'Not set')}`")
                st.write(f"**Max Cloud Cover:** `{filters.get('cloud_cover') * 100:.0f}%`" if filters.get('cloud_cover') is not None else "Not set")
                st.write(f"**Geometry/Area:** `{'Set' if filters.get('geometry') else 'Not set'}`")
            ...
            try:
                ...
                if count == 0:
                    ...
                    st.table(preview)
                    # ===== FIX: Removed the download button =====
            except Exception as e:
